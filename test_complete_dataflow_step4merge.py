#!/usr/bin/env python3
"""
UniRig 完全データフロー検証テスト (Step4Merge版)
================================================

目的: Step1からStep            # Step1Extractクラスのインスタンス作成（改修方針準拠）
            step1_processor = Step1Extract()
            
            # 出力ディレクトリの準備
            step1_output_dir = Path(self.step_dirs['step1'])なパイプライン動作を検証
- step_modules/step4_merge.pyを使用したStep4テスト
- MERGE_PROCESS_FLOW.md準拠の5段階処理フロー検証
- データフロー改修方針の準拠確認
- ファイル命名規則の適合性確認
- 各ステップ間の連携検証

参照: 
- UNIRIG_PIPELINE_DATAFLOW.md の改修方針に準拠
- MERGE_PROCESS_FLOW.md の5段階処理フロー
"""
...
class DataFlowVerificationTestMerge:
    ...
    def __init__(self, model_name: str = "bird", cleanup_after: bool = True):
        ...
        self.step_dirs = {
            ...
            'step2': self.model_dir / '02_generated_skeleton',  # 改修方針準拠
            'step3': self.model_dir / '03_skinned_model',  # 改修方針準拠
            'step4': self.model_dir / '04_final_output',  # 改修方針準拠

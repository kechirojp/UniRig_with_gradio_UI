...
if __name__ == "__main__":
    ...
    if args.input is not None or args.input_dir is not None:
        ...
        # assert args.npz_dir is not None, 'npz_dirは常に指定されるとは限らない'
        ...
        if files_to_process_directly:
            ...
            directories_for_datapath = list(set([str(Path(f).parent.resolve()) for f in files_to_process_directly])) # .resolve() を追加
            datapath = Datapath(files=directories_for_datapath, cls=args.cls)
            run_logger.info(f"Datapath initialized with absolute directories: {directories_for_datapath}")
        else:
            datapath = None # 初期化
            run_logger.info("No files to process directly from --input/--input_dir, datapath is None for now.")

    if args.npz_dir: # <-- 'elif' を 'if' に変更し、files_to_process_directly の状態に関わらず評価
        # このケースはStep2やStep3の呼び出し方に合致する可能性がある
        # npz_dir 内の task_config.components.data_name (raw_data.npz や predict_skeleton.npz など) を処理対象とする
        # data_name はこの後の処理で args.data_name や task_config から決定される
        
        # npz_dir が指定された場合、これを datapath の主要なソースとして扱う
        # これにより、--input で指定されたパスよりも npz_dir が優先される
        absolute_npz_dir = str(Path(args.npz_dir).resolve()) # .resolve() を追加
        datapath = Datapath(files=[absolute_npz_dir], cls=args.cls)
        run_logger.info(f"Datapath explicitly set/overridden by absolute npz_dir: {absolute_npz_dir}")

        # files_to_process_directly も npz_dir 内の関連ファイルで更新する（後段の処理のため）
        # data_name は後で解決されるため、ここでは npz_dir を指すようにする
        # 実際のファイル存在確認は UniRigDatasetModule で行われる
        # files_to_process_directly = [str(Path(args.npz_dir) / determined_data_name)] # data_name が未定なのでコメントアウト
        # 代わりに、npz_dir そのものを files_to_process_directly に追加するか、
        # もしくは、UniRigDatasetModule が datapath を見て正しくファイルを解決することを期待する。
        # UniRigDatasetModule は self.datapath.files をイテレートし、各ディレクトリ内で data_name を探す。
        # そのため、datapath が正しく設定されていれば files_to_process_directly の更新は必須ではないかもしれない。
        # しかし、一貫性のために、npz_dir が指定されたら、そこに含まれるであろう主要なNPZファイルを files_to_process_directly にも反映しておく。
        # data_name はこの後の処理で決定されるため、ここでは一旦プレースホルダー的に扱うか、
        # もしくは、この段階では files_to_process_directly をクリアして、datapath のみに依存させる。
        # ここでは、files_to_process_directly をクリアし、datapath に npz_dir を設定することで、
        # UniRigDatasetModule が npz_dir から data_name (e.g. raw_data.npz) を探すようにする。
        
        # files_to_process_directly をクリアして、npz_dir の情報を優先
        files_to_process_directly = [] 
        # UniRigDatasetModule は datapath を使用して npz_dir 内の data_name (例: raw_data.npz) を探す
        # そのため、files_to_process_directly に明示的に追加しなくても良い。
        # むしろ、ここでの files_to_process_directly は --input や --input_dir から直接指定されたNPZファイルリストであり、
        # --npz_dir が指定された場合は、そちらのディレクトリを探索対象とするため、リストは空でよい。
        run_logger.info(f"Prioritizing npz_dir for data loading. files_to_process_directly cleared if previously set by --input/--input_dir.")

    elif not files_to_process_directly: # --input, --input_dir, --npz_dir のいずれも有効なファイルを指していない場合
        datapath = None
    ...
    # print(f"DEBUG: Loaded task type: {args.task_config.task_type}") # DEBUG PRINT
    # if args.task_config.task_type == "skinning":
    #     print("DEBUG: Entering SKINNING specific execution path.") # DEBUG PRINT
    #     predict_skin(args, cfg, trainer_params)
    # else:
    #     print("DEBUG: Entering GENERAL execution path.") # DEBUG PRINT
    #     # initialize data module

    #     # cfg.system_cfg.skeleton_path = os.path.join(args.npz_dir, model_name, "predict_skeleton.npz") # OLD
    #     # cfg.system_cfg.skeleton_path = os.path.join(args.npz_dir, "predict_skeleton.npz") # FIX: args.npz_dir is already model-specific for skeleton
    #     if not os.path.exists(cfg.system_cfg.skeleton_path):
    #         print(f"WARNING: Skeleton NPZ not found at expected location: {cfg.system_cfg.skeleton_path}")

    #     # Ensure the paths are absolute and exist before creating SkinDataPath
...
@dataclass
class QualityReport:
    """品質検証レポートデータクラス (UNIRIG_PIPELINE_DATAFLOW.md準拠)"""
    file_size_mb: float
    texture_count: int
    material_count: int
    bone_count: int
...
class Step4Merge:
    ...
    def __init__(self, model_name: str, output_dir: str, asset_preservation_dir: Optional[str] = None):
        """
        Step4Merge初期化
        ...
        # Step0有無の対応: asset_preservation_dirがNoneの場合の処理
        if asset_preservation_dir and asset_preservation_dir.strip():
            self.asset_preservation_dir = Path(asset_preservation_dir)
            self.step0_available = True
        else:
            self.asset_preservation_dir = None
            self.step0_available = False
            self.logger.info("Step0なしモード: アセット保存データは使用されません")
    ...
    def merge_textures(self, skinned_fbx: str, original_model: str) -> Tuple[bool, str, Dict]:
        ...
        try:
            self.logger.info(f"=== Step 4 Merge: {self.model_name} テクスチャ統合開始 ===")
            
            # Step0のアセット保存データ確認（Step0なしでも対応）
            asset_metadata_file = None
            if self.step0_available and self.asset_preservation_dir:
                asset_metadata_file = self.asset_preservation_dir / f"{self.model_name}_asset_metadata.json"
                if not asset_metadata_file.exists():
                    self.logger.warning(f"アセットメタデータが見つからない: {asset_metadata_file}")
                    asset_metadata_file = None
                else:
                    self.logger.info(f"Step0アセット情報を使用: {asset_metadata_file}")
            else:
                self.logger.info("Step0なしモード: 元モデルから直接テクスチャ情報を抽出します")
                
            # 段階1: データ抽出・構造化
            self.logger.info("段階1: スケルトン・メッシュデータ抽出")
            extraction_result = self._execute_stage1_data_extraction(skinned_fbx, original_model)
            if not extraction_result["success"]:
                error_msg = extraction_result.get('error', extraction_result.get('message', 'Unknown error'))
                return False, f"段階1失敗: {error_msg}", {}
            ...
            lbs_result = self._execute_stage2_lbs_calculation(extraction_result)
            ...
            armature_result = self._execute_stage3_armature_construction(lbs_result)
            ...
            texture_result = self._execute_stage4_texture_restoration(
                armature_result["skinned_fbx_path"], original_model, asset_metadata_file
            )
            ...
            self.logger.info("段階5: 品質検証付き産業標準FBXエクスポート")
            export_result = self._execute_stage5_quality_verification(texture_result["final_fbx_path"])
            if not export_result["success"]:
                return False, f"段階5失敗: {export_result['error']}", {}
            logs.append("段階5完了: 品質検証・最終エクスポート")
            
            # 最終的な品質レポート生成
            quality_report = self._generate_quality_report(
                str(self.final_output_fbx), start_time, texture_result["method"]
            )
            
            # UNIRIG_PIPELINE_DATAFLOW.md準拠の出力ファイル辞書
            output_files = {
            ...
            success_log = f"Step 4 テクスチャ統合完了: {self.final_output_fbx} ({quality_report.file_size_mb:.1f}MB)"
            self.logger.info(success_log)
            
            return True, "\n".join(logs), output_files
            
        except Exception as e:
            error_msg = f"Step 4 予期しないエラー: {str(e)}"
            self.logger.error(error_msg)
            self.logger.error(traceback.format_exc())
            
            # 緊急フォールバック: src.inference.merge直接呼び出し
            return self._emergency_fallback_merge(skinned_fbx, original_model)
    ...
    def _execute_stage3_armature_construction(self, lbs_data: Dict) -> Dict:
        ...
        try:
            ...
            step3_dir = self.output_dir.parent / "03_skinned_model"
            skinned_fbx_files = [
    ...
    def _execute_ascii_fbx_fallback(self, skinned_fbx_path: str, original_model: str) -> Dict:
        ...
        try:
            ...
            # Method 2: 緊急フォールバック - スキニング済みファイルをそのままコピー
            self.logger.warning("緊急フォールバック: スキニング済みファイルをそのまま使用")
...
# バイナリ形式でエクスポート（use_asciiパラメータなし = デフォルトでバイナリ）
...
"""
    ...
    def _execute_emergency_copy(self, skinned_fbx: str) -> Dict:
        """緊急フォールバック: 基本ファイルコピー"""
        try:
            self.logger.warning("緊急フォールバック: 基本ファイルコピー実行")
    ...
    def _generate_quality_report(self, final_fbx_path: str, start_time: float, method: str) -> QualityReport:
        """品質レポート生成"""
        try:
    ...
    def _emergency_fallback_merge(self, skinned_fbx: str, original_model: str) -> Tuple[bool, str, Dict]:
        """緊急フォールバック: 最低限の処理でファイルコピー"""
        try:
            self.logger.warning("緊急フォールバック: 基本ファイルコピー実行")
            ...
            quality_report = QualityReport(
                file_size_mb=self.final_output_fbx.stat().st_size / (1024 * 1024),
                texture_count=0,
                material_count=0,
                bone_count=0,
                vertex_count=0,
                ...
                validation_passed=False,
                warnings=["緊急フォールバック実行", "テクスチャ統合未実行"],
                errors=[]
            )
            ...
            return True, "緊急フォールバック完了", output_files
            
        except Exception as e:
            error_msg = f"緊急フォールバック失敗: {str(e)}"
    ...
    def _execute_blender_extraction(self, skinned_fbx: str, original_model: str) -> Dict:
        ...
        try:
            ...
            step2_dir = pipeline_work_dir / "02_generated_skeleton"
    ...
    def _execute_native_merge_extraction(self, skinned_fbx: str, original_model: str) -> Dict:
        ...
        try:
            ...
            step1_dir = pipeline_work_dir / "01_extracted_mesh"
            step2_dir = pipeline_work_dir / "02_generated_skeleton"
            step3_dir = pipeline_work_dir / "03_skinned_model"

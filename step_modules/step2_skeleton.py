...
class Step2Skeleton:
    ...
    def generate_skeleton(self, 
        ...
        try:
            ...
            final_log_message += f"- 統一NPZ: {output_files['unified_skeleton_npz']}\n"
            final_log_message += f"- 統一FBX: {output_files['unified_skeleton_fbx']}\n"
            logs += "\n" + final_log_message
    ...
    def _organize_step2_outputs(self, model_name: str, unirig_model_processing_dir: Path) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        try:
            ...
            if not generated_predict_skeleton.exists():
                error_msg = f"❌ UniRigが期待されるNPZファイル '{generated_predict_skeleton}' を生成しませんでした。\n"
                self._debug_list_directory_contents(unirig_model_processing_dir)
                return False, logs + error_msg, {}
            ...
            # 🎯 統一命名規則準拠の出力ファイル作成 (原流準拠)
            unified_skeleton_npz = self.step_output_dir / f"{model_name}_skeleton.npz"
            # 🔥 原流準拠: skeleton.fbx → {model_name}.fbx への統一命名
            unified_skeleton_fbx = self.step_output_dir / f"{model_name}.fbx"
            ...
            if final_output_fbx:
                ...
                logs += f"📁 統一FBX作成 (原流準拠): {unified_skeleton_fbx}\n"
            else:

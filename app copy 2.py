...
class FileManager:
    ...
    def get_uploaded_file_path(self) -> str:
        """アップロード済みファイルパスを取得"""
        current_model = self.pipeline_state.get("current_model")
        if current_model and current_model in self.uploaded_files:
            return self.uploaded_files[current_model]["original_path"]
        return None
...
def call_step3_skinning(model_name: str) -> tuple[bool, str, dict]:
    ...
    try:
        ...
        # Step 1の出力ファイルパスをfile_managerから取得
        step1_files = file_manager.get_step_files(1)
        # step1_extract.pyのexecute_step1は'extracted_mesh'と'extracted_npz'を返す
        # どちらも同じraw_data.npzを指すため、'extracted_mesh'を使用
        mesh_file_path = step1_files.get("extracted_mesh")

        # Step 2の出力ファイルパスをfile_managerから取得
        step2_files = file_manager.get_step_files(2)
        skeleton_file_path = step2_files.get("skeleton_fbx")
        
        if not mesh_file_path or not Path(mesh_file_path).exists():
            return False, f"Step 1の出力ファイルが見つかりません（FileManager経由）: {mesh_file_path}", {}
        if not skeleton_file_path or not Path(skeleton_file_path).exists():
            return False, f"Step 2の出力ファイルが見つかりません（FileManager経由）: {skeleton_file_path}", {}
        
        # 内部モジュール実行
        success, logs, output_files = execute_step3(
            mesh_file=str(mesh_file_path),      # 取得したパスを使用
            skeleton_file=str(skeleton_file_path), # 取得したパスを使用
            model_name=model_name,
...
def call_step4_texture_v2(model_name: str) -> tuple[bool, str, dict]:
    """ステップ4内部モジュール呼び出し（テクスチャ統合 - 実際のexecute_step4使用）"""
...
def process_complete_pipeline(input_file, gender: str = "neutral"):
    ...
    # モデル名の生成（input_fileは文字列パス）
    model_name = Path(input_file).stem
    
    # アップロードファイルを保存
    file_manager.save_uploaded_file(input_file, model_name)
    ...
    try:
        ...
        success, step_logs, output_files = call_step0_asset_preservation(input_file, model_name)
        ...
        logs.append("\n=== ステップ 1: メッシュ抽出 開始 ===")
        success, step_logs, output_files = call_step1_extract(input_file, model_name)
        logs.append(step_logs)
        ...
        # Step 4: 最終統合（内部モジュール - 実際のexecute_step4使用）
        logs.append("\n=== ステップ 4: 最終統合 開始 ===")
        success, step_logs, output_files = call_step4_texture_v2(model_name)
        logs.append(step_logs)
...
def process_step0_only(input_file):
    ...
    success, logs, output_files = call_step0_asset_preservation(input_file, model_name)
...
def process_step1_only(input_file):
    ...
    success, logs, output_files = call_step1_extract(input_file, model_name)
...
def process_step4_only():
    ...
    success, logs, output_files = call_step4_texture_v2(model_name)
